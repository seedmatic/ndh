...
state_keys_dir="@perUserKeysDir@"
keys_yaml_file="@perUserKeysDir@.yaml"
ssh_keychain_keys=()
...
ssh_keychain_cleanup_cert() {
  ...
  cert_fp="$(ssh-keygen -Lf "$cert_path" 2>/dev/null | awk '/Public key:/ {print $4; exit}' || true)"
...
ssh_keychain_eval() {
  ...
  eval "$(keychain --quiet --eval "${ssh_keychain_keys[@]}")"
}
...
ssh_keychain_add_missing_keys() {
  ...
  for key_path in "${ssh_keychain_keys[@]}"; do
    ...
    ssh-add -q "$key_path" </dev/null >/dev/null 2>&1 || true
  done
}
...
ssh_keychain_init() {
  ...
  command -v keychain >/dev/null 2>&1 || return 0
...
unset -f \
  ...
  ssh_keychain_add_missing_keys \
  ssh_keychain_init
unset keys_yaml_file ssh_keychain_keys state_keys_dir
