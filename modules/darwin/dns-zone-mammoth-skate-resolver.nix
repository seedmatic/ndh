...
# resolver entry; `nameserver 127.0.0.1` + `port 5354` here means
# "send queries matching `*.mammoth-skate.test` to localhost:5354",
# which is where our dnsmasq is bound (see modules/.common.d/dnsmasq.nix).
# The unprivileged port lets dnsmasq run as the primary user instead
# of root (see modules/darwin/dnsmasq.nix UserName/GroupName).
#
...
in
{
  config = lib.mkIf (dnsZoneCfg.enable && zone != "") {
    environment.etc."resolver/${zone}".text = ''
      ...
      # ${zone} zone served by the local dnsmasq on
      # 127.0.0.1:5354 (see modules/.common.d/dns-zone-mammoth-skate.nix).
