...
let
  ...
  hostName = if (config ? limaHost && config.limaHost ? hostName && config.limaHost.hostName != null && config.limaHost.hostName != "")
             then config.limaHost.hostName
  ...
  makeScript = name: srcFile: pkgs.runCommand name { } ''
    install -Dm755 ${pkgs.replaceVars srcFile { inherit hostName systemAttr flakeRef; }} $out/bin/${name}
  '';
...
in {
  environment.systemPackages = [ manualSwitch manualBoot nixosRebuildGuest ];
