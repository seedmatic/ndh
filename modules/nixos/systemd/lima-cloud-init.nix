  ...
  trustedCaPublicKey = lib.removeSuffix "\n" (
    builtins.readFile (
      pkgs.runCommand "trusted-user-ca-public-key" { buildInputs = [ pkgs.yq-go ]; } ''
        yq -r '.authorities."mammoth-skate".public // ""' "${worktreePath.of "modules/home-manager/ssh.d/keys.yaml"}" > "$out"
      ''
    )
  );
  linuxBuilderPublicKey = lib.removeSuffix "\n" (
    builtins.readFile (
      pkgs.runCommand "linux-builder-public-key" { buildInputs = [ pkgs.yq-go ]; } ''
        yq -r '.keys."linux-builder".public // ""' "${worktreePath.of "modules/home-manager/ssh.d/keys.yaml"}" > "$out"
      ''
    )
  );
