#!/usr/bin/env bash
#
# Deliberately does NOT source the nix bash trampoline, unlike the other scripts
# in this tree (mk-disk-image-with-manifest.sh, erofs-store-image.d/*).  Tried on
# 2026-09-19 and reverted: this one runs INSIDE the nested build VM, and there
# the trampoline breaks twice over.
#
# (Note the placeholder name is spelled out rather than written in its @...@ form:
# replaceVars scans for those anywhere in the file, comments included, and fails
# the build when one has no matching attribute.)
#
#   - It requires the NDH bootstrap profile
#     (/nix/var/nix/profiles/per-user/root/io-seedmatic-ndh-bringup-runtime) and
#     errors when it is absent.  It cannot be present here: installing it is
#     something THIS script does, later, and into /mnt/zfs-root — so sourcing the
#     trampoline first is a circular dependency.
#   - The logger redirects FD 2, and in the nested VM nothing ever closes it, so
#     the relay never sees EOF and QEMU does not terminate — the build hangs after
#     "installation finished!".  Same family as the SIGPIPE the flake's
#     mkNdhNixBashTrampoline comment documents for the initrd.
#
# The trampoline is for scripts running on a host that already has the profile.
set -euxo pipefail
