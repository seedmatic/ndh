...
main() {
  ...
  if pool_imported "$PRIMARY_POOL"; then
    : "[zpool-init][INFO] primary pool already imported (${PRIMARY_POOL}), skipping provisioning"
  elif pool_discoverable "$PRIMARY_POOL"; then
    : "[zpool-init][INFO] primary pool discoverable (${PRIMARY_POOL}), importing with -N for normalization"
    for pool in "${EXPECTED_POOLS[@]}"; do
      import_pool_if_discoverable "$pool"
    done
  else
    : "[zpool-init][INFO] running disko configuration"
    if [ -f "${DISKO_NIX}" ]; then
      disko --mode format,mount "${DISKO_NIX}"
    else
      : "[zpool-init][ERROR] disko.nix not found at expected path"
      exit 1
    fi

    : "[zpool-init][INFO] unmounting ZFS datasets for expected pools"
    for pool in "${EXPECTED_POOLS[@]}"; do
  ...
  : "[zpool-init][INFO] normalizing ZFS mountpoint=legacy for explicit path datasets"
  for pool in "${EXPECTED_POOLS[@]}"; do
  ...
  : "[zpool-init][INFO] ensuring expected ZFS pools are imported"
  for pool in "${EXPECTED_POOLS[@]}"; do
    ...
    if ! pool_imported "$pool"; then
      : "[zpool-init][WARN] expected pool not imported: $pool"
    fi
  done

  : "[zpool-init][INFO] datastore provisioning complete"
}

ndh::logger:command:run "nixos.zfs.zpool-init" main "$@"
