...
- kpt packages live under `modules/nixos/incus-rke2-cluster/kpt-packages` with a root `Kptfile` describing the stack.
...
spec:
  ...
  git:
    ...
    branch: develop
    directory: modules/nixos/incus-rke2-cluster/kpt-packages
  deployment: false
----
...
kind: PackageVariant
metadata:
  name: bioskop-openebs-zfs
  namespace: porch-system
spec:
  upstream:
    repo: incus-kpt-catalog
    package: storage/openebs-zfs
    revision: v1
  downstream:
    repo: bioskop-cluster-deploy
    package: storage/openebs-zfs
  packageContext:
    data:
  ...
  injectors:
    - name: bioskop-storage-pool
      group: infra.nxmatic.dev
      version: v1alpha1
      kind: StoragePoolConfig
----
...
.. Apply the `incus-kpt-catalog` and `bioskop-cluster-deploy` `Repository` objects.
...
.. Apply the `bioskop-openebs-zfs` `PackageVariant` with cluster-specific setters/injectors.
