...
1. **Headscale Server & Client** ✅ (from `cloud-config.master.headscale.yaml`)
   - Deployed via kpt package: `kpt/cluster/mesh/headscale/`
...
2. **Envoy Gateway** ✅ (from `cloud-config.master.cilium.yaml`)
   - Deployed via kpt package: `kpt/cluster/networking/envoy-gateway/`
...
3. **Kube-VIP DaemonSet** ✅ (from `cloud-config.master.kube-vip.yaml`)
   - Deployed via kpt package: `kpt/cluster/ha/kube-vip/`
...
4. **Cilium Advanced Features** ✅ COMPLETE (from `cloud-config.master.cilium.yaml`)
   ...
   - ✅ Deployed via kpt package: `kpt/cluster/networking/cilium/`
...
upstream:
  ...
  git:
    ...
    directory: modules/nixos/incus-rke2-cluster/kpt/cluster/mesh/headscale
    ref: develop
pipeline:
  mutators:
...
**To**: Multiple files in `kpt/cluster/mesh/headscale/`

**File 1**: `kpt/cluster/mesh/headscale/namespace.yaml`
...
**File 2**: `kpt/cluster/mesh/headscale/configmaps.yaml`
...
[source,bash]
----
kpt fn eval kpt/cluster/mesh/headscale \
  --image ghcr.io/kptdev/krm-functions-catalog/apply-setters:v0.2 \
  -- \
  home-lan-lb-ip=192.168.1.193 \
  headscale-version=v0.27.0 \
