...
=== Phase 1: Foundation (Non-Critical Components)
...
1. **Headscale Server & Client** (from `cloud-config.master.headscale.yaml`)
   - Low risk, bioskop-specific only
   - Good learning opportunity for kpt workflows
   - No dependency on core cluster functionality

2. **Envoy Gateway** (from `cloud-config.master.cilium.yaml`)
   - Optional component, can be deployed post-bootstrap
   - Complex Job-based installation suits kpt approach
   - Independent of Cilium core functionality

**Success Criteria**: Components deploy successfully via kpt, cluster remains stable
...
[source,bash]
----
# After cluster starts, verify kpt is available
