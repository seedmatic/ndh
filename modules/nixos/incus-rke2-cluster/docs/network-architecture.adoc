...
|10.80.16.1 (fallback, NAT disabled)
...
NOTE: Route metrics ensure `lan0` (metric 100) is preferred over `wan0` (metric 9999) for internet traffic.
...
|**Gateway** |10.80.16.1 (Lima VM IP on bridge)
|**Static Leases** |MAC-based (52:54:00:02:00:xx → 10.80.16.1x)
|**NAT** |Enabled but unreliable for TCP (ICMP works)
|**IPv6** |fd42:21ea:8a29:f3c9::/64 with NAT
...
|Metric |9999 (fallback only)
|Gateway |10.80.16.1 (Lima VM) - NAT disabled for TCP reliability
|===

=== Rationale for LAN as Primary Gateway
...
**Solution**: Use home router (via `lan0`) as primary internet gateway:
- Home router provides stable, tested NAT
- Lima VM bridge (`wan0`) used only for cluster-internal traffic
- Route metrics ensure `lan0` is preferred (100 vs 9999)
...
wan0:
  dhcp4: true
  dhcp4-overrides:
    route-metric: 9999     # FALLBACK route (very low priority)
    use-dns: false         # Don't override LAN DNS
    use-routes: true       # Accept route but deprioritized
  optional: false
```
...
|100
|9999
|Lower = higher priority. lan0 preferred for all internet traffic

|`use-routes`
|true
|true
|Both accept default routes, metric determines preference
...
Check actual metrics on a running node:
```bash
ip route show default
# Expected output:
# default via 192.168.1.254 dev lan0 proto dhcp src 192.168.1.x metric 100
# default via 10.80.16.1 dev wan0 proto dhcp src 10.80.16.x metric 9999
```
...
WARNING: If metrics are inverted (lan0=9999, wan0=100), internet traffic will route through Lima VM NAT (unreliable TCP). Apply correct netplan config with `netplan apply`.

==== Troubleshooting Network Configuration

**Problem: Inverted Route Metrics**

Some nodes (peer1, peer2) may still have the old netplan configuration with inverted metrics:
```bash
# INCORRECT (old config):
default via 10.80.16.1 dev wan0 proto dhcp src 10.80.16.x metric 100    # wan is primary
default via 192.168.1.254 dev lan0 proto dhcp src 192.168.1.x metric 9999  # lan is fallback
```

These nodes will route internet traffic through Lima VM NAT (unreliable TCP), causing:
- Slow or failed container image pulls
- Timeouts connecting to external services (Docker Hub, GitHub, etc.)
- ICMP (ping) working but TCP failing
...
3. Verify metrics are now correct:
```bash
ssh lima-nerd-nixos "incus exec peer1 -- ip route show default"
# Should show:
# default via 192.168.1.254 dev lan0 ... metric 100      # LAN is primary
# default via 10.80.16.1 dev wan0 ... metric 9999        # wan is fallback
```
...
=== Primary Routing
- **Internet access**: via `lan0` interface using home router gateway (192.168.1.254)
- **Cluster communication**: via `wan0` interface within cluster network (10.80.16.0/21)
