...
```makefile
# Map network.mk variables to template variables
CLUSTER_LOADBALANCERS_CIDR := $(CLUSTER_LOADBALANCERS_NETWORK)
CLUSTER_VIRTUAL_CIDR := $(CLUSTER_VIP_NETWORK)  
CLUSTER_VIP_BRIDGE_CIDR := $(CLUSTER_VIP_NETWORK)
```
