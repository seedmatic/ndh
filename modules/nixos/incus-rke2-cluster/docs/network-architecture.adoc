...
' NAT / routing path (Strategy B: retain internal subnets)
VMWAN0 --> ENP : route/NAT (masquerade 10.80.24.0/21,\n10.80.31.0/24)
...
note bottom of ENP
	Outbound NAT applies here
	for internal 10.80.* ranges
end note
...
=== Diagram Legend
* macvlan: Container interface directly on parent host NIC (no bridge)
* Internal subnets (10.80.24.0/21, 10.80.31.0/24): Require NAT on host if not routable upstream
* NAT boundary: Implemented via nftables/iptables postrouting masquerade on `enp0s1`
* VIP subnet: Control plane virtual IP allocations (e.g. kube-vip)

=== Next Steps
If retaining internal subnets (Strategy B), ensure nft NAT rules:
```bash
sudo nft add table ip nat
sudo nft add chain ip nat postrouting '{ type nat hook postrouting priority 100 ; }'
sudo nft add rule ip nat postrouting ip saddr 10.80.24.0/21 oif enp0s1 masquerade
sudo nft add rule ip nat postrouting ip saddr 10.80.31.0/24 oif enp0s1 masquerade
```
Codify these in a NixOS module for persistence (recommended). The macvlan design keeps container isolation while allowing upstream access via controlled NAT.