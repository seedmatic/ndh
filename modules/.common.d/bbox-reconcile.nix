...
# (POST/PUT/DELETE on `/dhcp/clients/<id>`) but we have not yet
# captured the body shape the UI sends, so this script is diff-only
# until that's settled.  See `docs/bbox-api.adoc` for the full surface
# and probing convention.
#
# Inputs
...
#
# Comparison key is MAC address (the only identifier the bbox treats
# as primary).  Hostname or IP changes against the same MAC count as
# drift, not as missing/extra.
{
  config,
  lib,
  pkgs,
...
let
  ...
  # consumes directly.
  ignoredYaml = paths.catalogLanIgnoredReservations;
  ignoredJson =
  ...
  bboxReconcile = pkgs.writeShellApplication {
    name = "bbox-reconcile";
    ...
    text = ''
      ...
      SECRETS_FILE='${paths.repoSecrets}'
...
in
{
  config = lib.mkIf routerAvailable {
    environment.systemPackages = [ bboxReconcile ];
  };
}
