...
install_profile() {
  echo "[nerd-bringup-runtime] installing/refreshing profile ${profile_dir}" >&2
...
if [ "$image_build_context" = "1" ] && [ "$auto_install" = "1" ]; then
  echo "[nerd-bringup-runtime] image-build activation context detected; seeding profile ${profile_dir}" >&2
  install_profile
fi
...
if [ ! -d "$profile_bin" ]; then
  if [ "$auto_install" = "1" ]; then
    ...
    echo "[nerd-bringup-runtime][ERROR] required profile bin directory is missing: $profile_bin" >&2
    echo "[nerd-bringup-runtime][ERROR] install it first: ${install_hint}" >&2
    exit 1
  fi
fi
...
if [ -n "$missing" ]; then
  echo "[nerd-bringup-runtime][ERROR] missing required commands:$missing" >&2
  echo "[nerd-bringup-runtime][ERROR] reinstall profile: ${install_hint}" >&2
  exit 1
fi

if [ -n "$wrong_source" ]; then
  echo "[nerd-bringup-runtime][ERROR] required commands not sourced from profile:$wrong_source" >&2
  echo "[nerd-bringup-runtime][ERROR] reinstall/repair profile: ${install_hint}" >&2
  exit 1
fi
