...
NOTE: The module is **enabled by default** for all Darwin hosts via `modules/darwin/default.nix` (using `programs.githubMcpProxy.enable = lib.mkDefault true;`). Hosts can opt out explicitly.
