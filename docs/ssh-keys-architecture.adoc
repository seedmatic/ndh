...
The SSH key management system automates deployment of cryptographic keys and certificates from a SOPS-encrypted source (`keys.yaml`) to a runtime directory (`~/.local/state/ssh-key.d/`) at activation time. The pipeline ensures:
...
|Rsyncs keys + certificates to ~/.local/state/ssh-key.d/, generates agent-keys manifest

|Ensure Authorized Keys (Stage 4)
|Creates mutable ~/.ssh/authorized_keys file with proper permissions

|Runtime Directory
|`~/.local/state/ssh-key.d/` - Contains keys, certs, symlinks, and manifest
|===
...
* rsync tmp_keys_dir → ~/.local/state/ssh-key.d/ with:
...
|Output
|Keys + certs + symlinks in ~/.local/state/ssh-key.d/
...
Plaintext Profile YAML
{
  ...
  linux-builder: { type, public, private, authorities: { mammoth-skate: { ... } } }
      ...
      │  ├─ Extracts ~/.local/state/ssh-key.d/github-signing (private)
      │  ├─ Extracts ~/.local/state/ssh-key.d/github-signing.pub (public)
      │  ├─ Extracts ~/.local/state/ssh-key.d/rke2-cluster (private)
      │  ├─ Extracts ~/.local/state/ssh-key.d/rke2-cluster.pub (public)
      │  ├─ Extracts ~/.local/state/ssh-key.d/rke2-cluster-mammoth-skate-ca.pub (CA authority)
...
[source]
----
~/.local/state/ssh-key.d/               # XDG_STATE_HOME/ssh-key.d
...
[source,bash]
----
ls -la ~/.local/state/ssh-key.d/my-new-key*
...
[source,bash]
----
ls -la ~/.local/state/ssh-key.d/rke2-cluster-mammoth-skate-user-cert.pub
ssh-keygen -L -f ~/.local/state/ssh-key.d/rke2-cluster-cert.pub
----
...
bash modules/home-manager/ssh-key.d/ssh-extract-keys.sh /tmp/profile.yml /tmp/extracted/
ls -la /tmp/extracted/
----
...
bash modules/home-manager/ssh-key.d/deploy-ssh-keys.sh
ls -la ~/.local/state/ssh-key.d/
cat ~/.local/state/ssh-key.d/agent-keys  # Should list keys to load
----
